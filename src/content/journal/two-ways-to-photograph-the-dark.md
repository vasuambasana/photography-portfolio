---
title: "Two Ways to Photograph the Dark"
subtitle: "Twenty-five night frames, and two completely opposite strategies for the same problem."
type: behind-the-lens
date: 2026-09-16T11:00:00.000Z
coverImage: celestial-rhythms-over-the-pines
tags: [night, astrophotography, long-exposure, technique]
excerpt: "The night frames in this archive run from two seconds to six hundred and ninety-three. That range isn't indecision. It's two different answers to the same question about where light comes from."
featured: false
draft: false
relatedPhotos:
  [
    celestial-rhythms-over-the-pines,
    canopy-of-infinity,
    aisle-of-stars,
    moonlit-tapestry,
    symphony-in-steel-and-shadow,
  ]
---

There are **25 night photographs** in this archive. Sorted by shutter speed, they don't form
a gradient. They form two clusters with a gap in the middle, and the gap is the interesting
part.

## Strategy one: wait

Twelve of the night frames are **twenty to twenty-five second exposures**, at ISO 500 to
1600. All twelve were shot on phones (a Galaxy S22 Ultra, an S24 Ultra, a Z Fold6, a
Z Fold7) across eight separate nights between April 2023 and April 2026.

That is a phone's astrophotography mode: hold the shutter open long enough that the sensor
accumulates what the eye can't, and stack the result. It requires something to brace
against, because twenty-five seconds is far past anything a hand can hold. Frames like
[*Aisle of Stars*](/photo/aisle-of-stars), [*Canopy of Infinity*](/photo/canopy-of-infinity) and [*Celestial Amphitheater*](/photo/celestial-amphitheater) all come out of this
approach, all at the phone's 23mm-equivalent wide camera.

Then there is the outlier that makes the whole cluster look impatient.

[*Celestial Rhythms Over the Pines*](/photo/celestial-rhythms-over-the-pines), October 2021: **693 seconds**. Eleven and a half
minutes on a Canon EOS 700D with the 18-55mm kit lens, at f/3.5 and ISO 200. Long enough
that the stars stop being points and become arcs, the earth's rotation drawn directly onto
the sensor. It is the only frame in the archive from that camera, and it is by a factor of
twenty-eight the longest exposure here.

## Strategy two: don't wait

The Canon R5 Mark II night frames do the opposite thing entirely.

[*Moonlit Tapestry*](/photo/moonlit-tapestry) and [*Neon Devotion*](/photo/neon-devotion), both from the night of 3 January 2026, are **1/60s
and 1/100s at ISO 3200**, both at 200mm. [*Symphony in Steel and Shadow*](/photo/symphony-in-steel-and-shadow) comes from the same
night at **1/160s and ISO 5000**, filed under architecture rather than night, but shot in
the same dark.

These are hand-holdable shutter speeds. Nothing is braced, nothing is waiting. The light is
being bought with sensitivity rather than with time, and the price is noise instead of
motion blur.

At 200mm that isn't really a choice. The reciprocal rule puts the hand-holding floor around
1/200s, and anything longer would smear. So the ISO goes up until the shutter can come down.

## The gap in the middle

Between two seconds and twenty seconds there is nothing at all. The two-second frame itself,
[*Neon Arteries*](/photo/neon-arteries) at **ISO 16**, is the boundary marker, and it's a bright-city long exposure
rather than a dark one, dragging the shutter to smooth traffic into ribbons. ISO 16 is a
camera being told there is far too much light, not too little.

The absence is the point. Exposures in that middle band are the worst of both: long enough
to need a tripod, short enough not to gather much for the trouble. If the camera is already
braced, there's no reason to stop at eight seconds. If it isn't, there's no reason to go
past one.

## The practical version

Two questions, in this order:

1. **Is the camera braced?** If yes, the ceiling is whatever the subject allows: 25 seconds
   for static stars, several minutes if the trails are the point. Keep ISO low and let time
   do the work.
2. **If not, how long is the lens?** That sets the slowest usable shutter, and ISO has to
   rise to meet it. At 200mm that means ISO 3200 and up, and accepting the noise.

What doesn't appear anywhere in these 25 frames is a middle path. The archive is fairly
clear that there isn't one worth taking.
